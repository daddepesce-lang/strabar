...
export const db = {
  // Modello alcolemico (puro, testato a parte): vedi src/lib/bac.js. Innestato qui con lo
  // spread così tutti i chiamanti esistenti (db.calculatePeakBAC, db.calculateBACCurve, …)
  ...
  // Profili del ramo LOCALE (demo senza Supabase): lì la "tabella" è un array in
  // localStorage, quindi leggerla intera non costa egress. Nel ramo Supabase si usano
  // sempre query mirate (getPublicProfiles / isUsernameTaken).
  _localProfiles() {
    if (typeof window === 'undefined') return [];
    return getStored('sb_profiles') || [];
  },

  // Colonne PUBBLICHE di un profilo: tutto ciò che serve a mostrare qualcuno agli altri
  // (nome pubblico, avatar, badge premium, gate delle classifiche) e NIENTE ALTRO.
  // Fuori restano di proposito: display_name è qui perché publicName() lo usa quando la
  // modalità-nome è "nome reale", ma weight, sex, notif_prefs, is_admin, account_type,
  // consent_version, tos_accepted_at NON escono più verso i client.
  // Prima si faceva `select('*')` su TUTTI i profili ad ogni apertura della home:
  //   • egress lineare sul numero di iscritti (cresce da solo mentre l'app cresce);
  //   • ogni client riceveva il nome REALE anche di chi ha scelto un alias, più peso e
  //     sesso di tutti — cioè il contrario di quello che promette "Come compaio agli altri".
  PUBLIC_PROFILE_COLS: 'id, username, display_name, alias, name_mode, use_username, avatar_url, is_premium, public_leaderboard, show_bac_public',

  // Profili PUBBLICI cercati per chiave esatta: id, username o nome visualizzato.
  // Una sola query per lotto (IN (...)), solo le righe che servono davvero. Sostituisce
  // getAllProfiles(): i chiamanti sanno sempre CHI gli serve (autori dei cheers, compagni
  // taggati), quindi non c'è motivo di scaricare la rubrica intera.
  async getPublicProfiles({ ids = [], usernames = [], names = [] } = {}) {
    const uniq = (a) => [...new Set(a.filter(Boolean))];
    const idList = uniq(ids), userList = uniq(usernames), nameList = uniq(names);
    if (!idList.length && !userList.length && !nameList.length) return [];
    if (isSupabaseConfigured) {
      const ors = [];
      if (idList.length) ors.push(`id.in.(${idList.join(',')})`);
      if (userList.length) ors.push(`username.in.(${userList.map((u) => `"${u}"`).join(',')})`);
      if (nameList.length) ors.push(`display_name.in.(${nameList.map((n) => `"${n.replace(/"/g, '')}"`).join(',')})`);
      const { data, error } = await supabase
        .from('profiles')
        .select(this.PUBLIC_PROFILE_COLS)
        .or(ors.join(','))
        .limit(200);
      if (error) throw error;
      return data || [];
    ...
    return (getStored('sb_profiles') || []).filter((p) => (
      idList.includes(p.id) || userList.includes(p.username) ||
      lowerNames.includes((p.display_name || '').toLowerCase())
    ));
  },

  // Username già preso da QUALCUN ALTRO? Una riga, non la tabella intera.
  async isUsernameTaken(username, exceptId = null) {
    const u = (username || '').trim().toLowerCase();
    if (!u) return false;
    if (isSupabaseConfigured) {
      let q = supabase.from('profiles').select('id').eq('username', u).limit(1);
    ...
    return (getStored('sb_profiles') || []).some((p) => p.username === u && p.id !== exceptId);
  },
  ...
  async getEvents() {
    ...
    const profiles = this._localProfiles();
    return events
      .map((e) => {
