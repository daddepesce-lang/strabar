...
import { QUICK_DRINKS, EXTRA_DRINKS, BEER_FAMILIES, withBuiltinFamilies } from '@/lib/drinks';
...
const STATIC = withBuiltinFamilies({ quick: QUICK_DRINKS, extra: EXTRA_DRINKS, beerFamilies: BEER_FAMILIES });
...
export function useDrinkCatalog() {
  ...
  useEffect(() => {
    ...
    if (cached?.v) setCat(withBuiltinFamilies(cached.v));
    ...
    if (stale) {
      db.getDrinkCatalog({ force: true })
        .then((c) => { if (alive && c && c.quick && c.extra && c.beerFamilies) setCat(withBuiltinFamilies(c)); })
        .catch(() => {});
    }
    return () => { alive = false; };
  }, []);
