...
import { computePrice, defaultOptions } from '@/lib/venuePricing';

export const runtime = 'nodejs';

// Invio ORDINI del locale SENZA pagamento online: ogni voce del carrello diventa un
// ordine "pending". Fattura e incasso si gestiscono fuori app; l'admin attiva l'ordine
// da /admin (Area locali → Ordini → "Attiva"). Il prezzo è AUTOREVOLE lato server.
export async function POST(req) {
  ...
  for (const it of items.slice(0, 10)) {
    ...
    const { error: orderErr } = await admin.from('venue_orders').insert({
  ...
  if (!count) return NextResponse.json({ error: 'Nessun articolo valido nel carrello' }, { status: 400 });
